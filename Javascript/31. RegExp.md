...
`{m,n}`은 앞선 패턴이 최소 m번, 최대 n번 반복되는 문자열을 의미한다. 컴마 뒤에 공백이 있으면 정상 동작하지 않으므로 주의하기 바란다.
...
`+`는 앞선 패턴이 최소 한번 이상 반복되는 문자열을 의미한다. 즉, `+`는 `{1,}`과 같다. 아래 예제의 경우, 앞선 패턴은 A이므로 `A+`는 A만으로 이루어진 문자열(‘A, ‘AA’, ‘AAA’, …)을 의미한다.

```
const target = 'A AA B BB Aa Bb';

// 'A'가 최소 한번 이상 반복되는 문자열('A, 'AA', 'AAA', ...)을 모두 검색
const regExp = /A+/g;

target.match(regExp); // -> ["A", "AA", "A"]
```

`?`는 앞선 패턴이 최대 한번(0번 포함) 이상 반복되는 문자열을 의미한다. 즉, `?`는 `{0,1}`과 같다. 아래 예제의 경우, `A?`는 ‘A’가 최대 한번(0번 포함) 이상 반복하는 문자열(‘’, ‘A)을 의미한다.
